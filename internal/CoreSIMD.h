...
bool Traverse_MicroTree_WithStack_AnyHit(const float ro[3], const float rd[3], int i, const mbvh_node_t *mnodes,
                                         uint32_t node_index, const tri_accel_t *tris, const tri_mat_data_t *materials,
                                         const uint32_t *tri_indices, int obj_index, hit_data_t<S> &inter);
...
template <int S>
void IntersectAreaLights(const ray_packet_t<S> &r, const simd_ivec<S> &ray_mask, const light_t lights[],
...
void ShadeSurface(const simd_ivec<S> &px_index, const pass_info_t &pi, const float *halton, const hit_data_t<S> &inter,
                  const ray_packet_t<S> &ray, const scene_data_t &sc, uint32_t node_index,
                  const Ref::TextureAtlasBase *tex_atlases[], simd_fvec<S> out_rgba[4],
                  simd_ivec<S> out_secondary_masks[], ray_packet_t<S> out_secondary_rays[],
...
#define _dot(x, y) ((x)[0] * (y)[0] + (x)[1] * (y)[1] + (x)[2] * (y)[2])
    ...
    const simd_fvec<S> det = _dot(r.d, tri.n_plane);
    const simd_fvec<S> dett = tri.n_plane[3] - _dot(r.o, tri.n_plane);
    ...
    const simd_fvec<S> p[3] = {det * r.o[0] + dett * r.d[0], det * r.o[1] + dett * r.d[1],
                               det * r.o[2] + dett * r.d[2]};
...
template <int S> simd_fvec<S> fresnel_dielectric_cos(const simd_fvec<S> &cosi, const simd_fvec<S> &eta) {
    ...
    g = sqrt(g);
    simd_fvec<S> A = (g - c) / (g + c);
    simd_fvec<S> B = (c * (g + c) - 1) / (c * (g - c) + 1);
            ...
            if (!is_leaf_node(nodes[cur])) {
                ...
                for (uint32_t i = prim_index; i < prim_index + nodes[cur].prim_count; i++) {
                    ...
                    const bool hit_found = Traverse_MicroTree_WithStack_AnyHit(
                        _r, bbox_mask, nodes, m.node_index, tris, materials, tri_indices, int(mi_indices[i]), inter);
                    res |= hit_found;
                }
            }
        }
        ...
        TRAVERSE:
            if (!is_leaf_node(nodes[cur.index])) {
                ...
                for (uint32_t j = prim_index; j < prim_index + nodes[cur.index].child[1]; j++) {
                    ...
                    const bool hit_found = Traverse_MicroTree_WithStack_AnyHit(
                        tr_ro, tr_rd, ri, nodes, m.node_index, tris, materials, tri_indices, int(mi_indices[j]), inter);
                    res |= hit_found;
                }
            }
        }
    ...
    while (st.index < st.num) {
        ...
        while (stack_size) {
            ...
            if (!is_leaf_node(nodes[cur])) {
                ...
                res |= IntersectTris_ClosestHit(r, st.queue[st.index].mask, tris, tri_start, tri_end, obj_index, inter);
            }
        }
        st.index++;
    }
...
bool Ray::NS::Traverse_MicroTree_WithStack_ClosestHit(const float ro[3], const float rd[3], int ri,
    ...
    TRAVERSE:
        if (!is_leaf_node(nodes[cur.index])) {
            ...
            res |= IntersectTris_ClosestHit(ro, rd, ri, tris, tri_start, tri_end, obj_index, inter);
        }
    }
        ...
        while (stack_size) {
            ...
            if (!is_leaf_node(nodes[cur])) {
                ...
                const bool hit_found =
                    IntersectTris_AnyHit(r, st.queue[st.index].mask, tris, tri_start, tri_end, obj_index, inter);
                /*if (hit_found) {
    ...
    TRAVERSE:
        if (!is_leaf_node(nodes[cur.index])) {
            ...
            const int tri_start = nodes[cur.index].child[0] & PRIM_INDEX_BITS,
                      tri_end = tri_start + nodes[cur.index].child[1];
            const bool hit_found =
                IntersectTris_AnyHit(ro, rd, ri, tris, materials, tri_indices, tri_start, tri_end, obj_index, inter);
            /*if (hit_found) {
                const bool is_backfacing = inter.prim_index < 0;
                const uint32_t prim_index = is_backfacing ? -inter.prim_index - 1 : inter.prim_index;
...
void Ray::NS::Sample_GGXRefraction_BSDF(const simd_fvec<S> T[3], const simd_fvec<S> B[3], const simd_fvec<S> N[3],
    ...
    if (cost2_positive.not_all_zeros()) {
        ...
        world_from_tangent(T, B, N, refr_dir_ts, out_V);
        Evaluate_GGXRefraction_BSDF(view_dir_ts, sampled_normal_ts, refr_dir_ts, roughness2, eta, refr_col, out_color);
    }
}
...
Ray::NS::simd_fvec<S> Ray::NS::ComputeVisibility(const simd_fvec<S> p[3], const simd_fvec<S> d[3], simd_fvec<S> dist,
    ...
    ITERATE_3({ sh_r.o[i] = p[i]; })
    ITERATE_3({ sh_r.d[i] = d[i]; })
    ...
    while (keep_going.not_all_zeros()) {
        ...
        if (sc.mnodes) {
            Traverse_MacroTree_WithStack_AnyHit(sh_r, keep_going, sc.mnodes, node_index, sc.mesh_instances,
                                                sc.mi_indices, sc.meshes, sc.transforms, sc.tris, sc.tri_materials,
                                                sc.tri_indices, sh_inter);
        } else {
            Traverse_MacroTree_WithStack_AnyHit(sh_r, keep_going, sc.nodes, node_index, sc.mesh_instances,
                                                sc.mi_indices, sc.meshes, sc.transforms, sc.tris, sc.tri_materials,
                                                sc.tri_indices, sh_inter);
        }
...
template <int S>
void Ray::NS::IntersectAreaLights(const ray_packet_t<S> &r, const simd_ivec<S> &ray_mask, const light_t lights[],
                                  Span<const uint32_t> visible_lights, const transform_t transforms[],
                                  hit_data_t<S> &inout_inter) {
...
void Ray::NS::ShadeSurface(const simd_ivec<S> &px_index, const pass_info_t &pi, const float *halton,
    ...
    { // Fetch planar normal
        const float *tri_n_planes = &sc.tris[0].n_plane[0];
        const int tri_n_planes_stride = sizeof(tri_accel_t) / sizeof(float);

        ITERATE_3({ plane_N[i] = gather(tri_n_planes, prim_index * tri_n_planes_stride + i); })
    }
    ...
    { // Sample materials
        ...
        while (index != num) {
            ...
            if (mat->type == DiffuseNode) {
...
#endif
                ITERATE_3(
                    { where(ray_queue[index], col[i]) += mis_weight * mat->strength * mix_weight * base_color[i]; })
                ...
                const simd_ivec<S> sample_spec_lobe = ~sample_diff_lobe & (spec_depth < pi.settings.max_spec_depth) &
                                                      (total_depth < pi.settings.max_total_depth) &
                ...
                const simd_ivec<S> sample_coat_lobe =
                    ~sample_diff_lobe & ~sample_spec_lobe & (spec_depth < pi.settings.max_spec_depth) &
                    (total_depth < pi.settings.max_total_depth) &
                ...
                const simd_ivec<S> sample_trans_lobe =
                    ~sample_diff_lobe & ~sample_spec_lobe & ~sample_coat_lobe &
                ...
                if (sample_trans_lobe.not_all_zeros()) {
                    ...
                    const simd_ivec<S> use_trans_spec_lobe = simd_cast(mix_rand < fresnel) & sample_trans_lobe;
                    if (use_trans_spec_lobe.not_all_zeros()) {
                        ...
                        where(use_trans_spec_lobe, new_ray.ray_depth) = ray.ray_depth + 0x00000100;

                        ITERATE_3({ where(use_trans_spec_lobe, new_ray.o[i]) = new_p[i]; })

                        ITERATE_3({
                            where(use_trans_spec_lobe, new_ray.dd_dx[i]) =
                                surf_der.dd_dx[i] -
                                2 * (dot(I, plane_N) * surf_der.dndx[i] + surf_der.ddn_dx[i] * plane_N[i]);
                        })
                        ITERATE_3({
                            where(use_trans_spec_lobe, new_ray.dd_dy[i]) =
                                surf_der.dd_dy[i] -
                                2 * (dot(I, plane_N) * surf_der.dndy[i] + surf_der.ddn_dy[i] * plane_N[i]);
                        })
                    }

                    const simd_ivec<S> use_trans_refr_lobe = ~use_trans_spec_lobe & sample_trans_lobe;
                    if (use_trans_refr_lobe.not_all_zeros()) {
                        ...
                        where(use_trans_refr_lobe, new_ray.ray_depth) = ray.ray_depth + 0x00010000;

                        ITERATE_4({ where(use_trans_refr_lobe, F[i]) = _F[i]; })
                        ITERATE_3({ where(use_trans_refr_lobe, V[i]) = _V[i]; })

                        ITERATE_3({ where(use_trans_refr_lobe, new_ray.o[i]) = new_p[i]; })
                        ...
                        ITERATE_3({
                            where(use_trans_refr_lobe, new_ray.dd_dx[i]) =
                                eta * surf_der.dd_dx[i] - (m * surf_der.dndx[i] + dmdx * plane_N[i]);
                        })
                        ITERATE_3({
                            where(use_trans_refr_lobe, new_ray.dd_dy[i]) =
                                eta * surf_der.dd_dy[i] - (m * surf_der.dndy[i] + dmdx * plane_N[i]);
                        })
                    }
