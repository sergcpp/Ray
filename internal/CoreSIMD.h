...
void Ray::NS::Sample_PrincipledClearcoat_BSDF(const simd_fvec<S> T[3], const simd_fvec<S> B[3], const simd_fvec<S> N[3],
    ...
    if (clearcoat_roughness2 * clearcoat_roughness2 < 1e-7f) {
        ...
        const simd_fvec<S> FH = (fresnel_dielectric_cos(dot3(out_V, N), simd_fvec<S>{clearcoat_ior}) - clearcoat_F0) /
                                (1.0f - clearcoat_F0);
...
void Ray::NS::SampleLatlong_RGBE(const Ref::TexStorageRGBA &storage, const uint32_t index, const simd_fvec<S> dir[3],
    ...
    simd_fvec<S> theta, phi = 0.0f;

    where(r > FLT_EPS, phi) = clamp(safe_div_pos(dir[0], r), -1.0f, 1.0f);

    ITERATE(S, {
        theta[i] = std::acos(y[i]) / PI;
        phi[i] = std::acos(phi[i]) + y_rotation;
    })
    where(phi < 0.0f, phi) += 2 * PI;
    where(phi > 2 * PI, phi) -= 2 * PI;

    simd_fvec<S> u = 0.5f * phi / PI;
...
Ray::NS::simd_fvec<S> Ray::NS::ComputeVisibility(const simd_fvec<S> p[3], const simd_fvec<S> d[3], simd_fvec<S> dist,
    ...
    while (keep_going.not_all_zeros()) {
        ...
        simd_fvec<S> p1[3] = {0.0f, 0.0f, 0.0f}, p2[3] = {1.0f, 0.0f, 0.0f}, p3[3] = {0.0f, 0.0f, 1.0f}, n1[3] = {},
                     n2[3] = {}, n3[3] = {}, u1[2] = {}, u2[2] = {}, u3[2] = {};
        ...
        const simd_fvec<S> sh_plane_N[3] = {dot3(sh_N, inv_xform1), dot3(sh_N, inv_xform2), dot3(sh_N, inv_xform3)};
        ...
        // simd_fvec<S> _dot_I_N = dot3(I, sh_N);
...
void Ray::NS::ComputeDerivatives(const simd_fvec<S> I[3], const simd_fvec<S> &t, const simd_fvec<S> do_dx[3],
    ...
    out_der.ddn_dx = dot3(out_der.dd_dx, plane_N) + dot3(I, out_der.dndx);
    out_der.ddn_dy = dot3(out_der.dd_dy, plane_N) + dot3(I, out_der.dndy);
}
    ...
    while (index != num) {
        ...
        if (l.type == LIGHT_TYPE_SPHERE) {
            ...
            const simd_fvec<S> lp[3] = {
                l.line.pos[0] + normal[0] * l.line.radius + (r2 - 0.5f) * light_dir[0] * l.line.height,
                l.line.pos[1] + normal[1] * l.line.radius + (r2 - 0.5f) * light_dir[1] * l.line.height,
                l.line.pos[2] + normal[2] * l.line.radius + (r2 - 0.5f) * light_dir[2] * l.line.height};
            ...
            ITERATE_3({ where(ray_queue[index], ls.L[i]) = safe_div_pos(to_light[i], ls.dist); })
            ...
            SampleLatlong_RGBE(*static_cast<const Ref::TexStorageRGBA *>(textures[0]), sc.env->env_map, ls.L, PI,
...
void Ray::NS::ShadeSurface(const simd_ivec<S> &px_index, const pass_info_t &pi, const float *halton,
    ...
    if (ino_hit.not_all_zeros()) {
        ...
        if (pi.should_add_environment()) {
            ...
            if (env_map != 0xffffffff) {
                SampleLatlong_RGBE(*static_cast<const Ref::TexStorageRGBA *>(textures[0]), env_map, ray.d, PI, ino_hit,
                                   env_col);
    ...
    // lambda -= fast_log2(abs(dot3(I, plane_N)));
#endif
    ...
    if (is_mix_mat.not_all_zeros()) {
...
#ifdef USE_RAY_DIFFERENTIALS
                const simd_fvec<S> base_lod =
                    get_texture_lod(textures, first_t, surf_der.duv_dx, surf_der.duv_dy, ray_queue[index]);
#else
        ...
        simd_fvec<S> RR = fresnel_dielectric_cos(dot3(I, N), eta);
            ...
            if (mat->type == DiffuseNode) {
                ...
                if (eval_light.not_all_zeros()) {
                    ...
                    ITERATE_3({
                        const simd_fvec<S> temp =
                            ls.col[i] * spec_col[i] * safe_div_pos(mix_weight * mis_weight, ls.pdf);
                        where(eval_light, sh_r.c[i]) = ray.c[i] * temp;
                        where(eval_light & ~ls.cast_shadow, col[i]) += temp;
                    })
                ...
                if (mat->flags & MAT_FLAG_SKY_PORTAL) {
                    ...
                    if (sc.env->env_map != 0xffffffff) {
                        ...
                        SampleLatlong_RGBE(*static_cast<const Ref::TexStorageRGBA *>(textures[0]), sc.env->env_map,
                                           ls.L, PI, ray_queue[index], tex_col);
                        ITERATE_3({ env_col[i] *= tex_col[i]; })
                    }
                ...
                if (mat->textures[METALLIC_TEXTURE] != 0xffffffff) {
                    ...
                    const simd_fvec<S> metallic_lod =
                        get_texture_lod(textures, metallic_tex, surf_der.duv_dx, surf_der.duv_dy, ray_queue[index]);
#else
                    const simd_fvec<S> metallic_lod = get_texture_lod(textures, metallic_tex, lambda, ray_queue[index]);
#endif

                    simd_fvec<S> metallic_color[4];
                    SampleBilinear(textures, metallic_tex, uvs, simd_ivec<S>(metallic_lod), ray_queue[index],
                                   metallic_color);
                ...
                const simd_fvec<S> FN =
                    (fresnel_dielectric_cos(dot3(I, N), simd_fvec<S>{spec_ior}) - spec_F0) / (1.0f - spec_F0);
                ...
                const simd_fvec<S> fresnel = fresnel_dielectric_cos(dot3(I, N), 1.0f / eta);
                ...
                if (eval_light.not_all_zeros()) {
                    ...
                    if (eval_spec_lobe.not_all_zeros()) {
                        ...
                        Evaluate_GGXSpecular_BSDF(view_dir_ts, sampled_normal_ts, light_dir_ts, max(alpha_x, 1e-7f),
                                                  max(alpha_y, 1e-7f), simd_fvec<S>{spec_ior}, simd_fvec<S>{spec_F0},
                                                  spec_tmp_col, spec_col);

                        where(eval_spec_lobe, bsdf_pdf) += specular_weight * spec_col[3];

                        ITERATE_3({ where(eval_spec_lobe, lcol[i]) += safe_div_pos(ls.col[i] * spec_col[i], ls.pdf); })
                    }
                    ...
                    if (eval_coat_lobe.not_all_zeros()) {
                        ...
                        ITERATE_3({
                            where(eval_coat_lobe, lcol[i]) +=
                                safe_div_pos(0.25f * ls.col[i] * clearcoat_col[i], ls.pdf);
                        })
                    }
                    ...
                    if (eval_refr_spec_lobe.not_all_zeros()) {
                        ...
                        ITERATE_3({
                            where(eval_refr_spec_lobe, lcol[i]) +=
                                ls.col[i] * spec_col[i] * safe_div_pos(fresnel, ls.pdf);
                        })
                    }
                    ...
                    if (eval_refr_trans_lobe.not_all_zeros()) {
                        ...
                        ITERATE_3({
                            where(eval_refr_trans_lobe, lcol[i]) +=
                                ls.col[i] * refr_col[i] * safe_div_pos(1.0f - fresnel, ls.pdf);
                        })
                    }
                ...
                if (sample_trans_lobe.not_all_zeros()) {
                    ...
                    if (sample_trans_spec_lobe.not_all_zeros()) {
                        ...
                        Sample_GGXSpecular_BSDF(T, B, N, I, roughness, simd_fvec<S>{0.0f} /* anisotropic */,
                                                simd_fvec<S>{1.0f} /* ior */, simd_fvec<S>{0.0f} /* F0 */,
                                                _spec_tmp_col, rand_u, rand_v, V, F);
    ...
    if (secondary_mask.not_all_zeros()) {
        ITERATE_3({ new_ray.c[i] = safe_div_pos(new_ray.c[i], 1.0f - q); })

        // TODO: get rid of this!
        ITERATE_3({ where(~secondary_mask, new_ray.d[i]) = 0.0f; })
