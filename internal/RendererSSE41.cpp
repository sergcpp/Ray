...
template void SampleNearest<RPSize>(const Ref::TexStorageBase *const textures[], const uint32_t index,
                                    const simd_fvec<RPSize> uvs[2], const simd_fvec<RPSize> &lod,
                                    const simd_ivec<RPSize> &mask, simd_fvec<RPSize> out_rgba[4]);
template void SampleBilinear<RPSize>(const Ref::TexStorageBase *const textures[], const uint32_t index,
                                     const simd_fvec<RPSize> uvs[2], const simd_ivec<RPSize> &lod,
                                     const simd_ivec<RPSize> &mask, simd_fvec<RPSize> out_rgba[4]);
template void SampleTrilinear<RPSize>(const Ref::TexStorageBase *const textures[], const uint32_t index,
                                      const simd_fvec<RPSize> uvs[2], const simd_fvec<RPSize> &lod,
                                      const simd_ivec<RPSize> &mask, simd_fvec<RPSize> out_rgba[4]);
template void SampleLatlong_RGBE<RPSize>(const Ref::TexStorageRGBA &atlas, uint32_t index,
...
class Renderer : public RendererSIMD<RPDimX, RPDimY> {
  public:
    ...
    eRendererType type() const override { return eRendererType::SIMD_SSE41; }
};
