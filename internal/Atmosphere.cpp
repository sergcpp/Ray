...
const Ref::fvec2 WrenningePhaseParameters = Ref::fvec2(-0.2f, 0.8f);
...
Ref::fvec4 PhaseWrenninge(float mu) {
    Ref::fvec4 phase = 0.0f;
...
float GetLightEnergy(const float dl, const float dC, const Ref::fvec4 &phase_probability) {
    // Wrenninge multi scatter approximation
    const auto exp_scale = Ref::fvec4(0.8f, 0.1f, 0.002f, 0.0f);
    const auto total_scale = Ref::fvec4(2.0f, 0.8f, 0.4f, 0.0f);
    const Ref::fvec4 intensity_curve = exp(-dl * exp_scale);
...
force_inline Ref::fvec4 AtmosphereDensity(const atmosphere_params_t &params, const float h) {
#if 1 // expf bug workaround (fp exception on unused simd lanes)
    const Ref::fvec4 density_rayleigh = exp(Ref::fvec4{-fmaxf(0.0f, h / params.rayleigh_height)});
    const Ref::fvec4 density_mie = exp(Ref::fvec4{-fmaxf(0.0f, h / params.mie_height)});
#else
...
struct atmosphere_medium_t {
    Ref::fvec4 scattering;
    Ref::fvec4 absorption;
    Ref::fvec4 extinction;

    Ref::fvec4 scattering_mie;
    Ref::fvec4 absorption_mie;
    Ref::fvec4 extinction_mie;

    Ref::fvec4 scattering_ray;
    Ref::fvec4 absorption_ray;
    Ref::fvec4 extinction_ray;

    Ref::fvec4 scattering_ozo;
    Ref::fvec4 absorption_ozo;
    Ref::fvec4 extinction_ozo;
};

force_inline atmosphere_medium_t SampleAtmosphereMedium(const atmosphere_params_t &params, const float h) {
    const Ref::fvec4 local_density = AtmosphereDensity(params, h);

    atmosphere_medium_t s;

    s.scattering_mie = local_density.get<1>() * Ref::fvec4{params.mie_scattering, Ref::vector_aligned};
    s.absorption_mie = local_density.get<1>() * Ref::fvec4{params.mie_absorption, Ref::vector_aligned};
    s.extinction_mie = local_density.get<1>() * Ref::fvec4{params.mie_extinction, Ref::vector_aligned};

    s.scattering_ray = local_density.get<0>() * Ref::fvec4{params.rayleigh_scattering, Ref::vector_aligned};
    s.absorption_ray = 0.0f;
    s.extinction_ray = s.scattering_ray + s.absorption_ray;

    s.scattering_ozo = 0.0;
    s.absorption_ozo = local_density.get<2>() * Ref::fvec4{params.ozone_absorbtion, Ref::vector_aligned};
...
Ref::fvec4 SampleTransmittanceLUT(Span<const float> lut, Ref::fvec2 uv) {
    uv = uv * Ref::fvec2(TRANSMITTANCE_LUT_W, TRANSMITTANCE_LUT_H);
    auto iuv0 = Ref::ivec2(uv);
...
Ref::fvec4 SampleMultiscatterLUT(Span<const float> lut, Ref::fvec2 uv) {
    uv = uv * Ref::fvec2(MULTISCATTER_LUT_RES);
    auto iuv0 = Ref::ivec2(uv);
...
Ref::fvec4 SampleWeatherTex(Ref::fvec2 uv) {
    uv = uv * Ref::fvec2(WEATHER_TEX_RES);
    auto iuv0 = Ref::ivec2{uv};
    iuv0 = clamp(iuv0, Ref::ivec2{0, 0}, Ref::ivec2{WEATHER_TEX_RES - 1});
...
float GetDensityHeightGradientForPoint(float height, float cloud_type) {
    const auto stratusGrad = Ref::fvec4(0.02f, 0.05f, 0.09f, 0.11f);
    const auto stratocumulusGrad = Ref::fvec4(0.02f, 0.2f, 0.48f, 0.625f);
    const auto cumulusGrad = Ref::fvec4(0.01f, 0.0625f, 0.78f, 1.0f);
    ...
    Ref::fvec4 cloudGradient = stratusGrad * stratus + stratocumulusGrad * stratocumulus + cumulusGrad * cumulus;
...
float Sample3dNoiseTex(Ref::fvec4 uvw) {
    uvw *= NOISE_3D_RES;
    Ref::ivec4 iuvw0 = Ref::ivec4(uvw);
    iuvw0 = clamp(iuvw0, Ref::ivec4{0}, Ref::ivec4{NOISE_3D_RES - 1});
    const Ref::ivec4 iuvw1 = (iuvw0 + 1) & Ref::ivec4{NOISE_3D_RES - 1};
...
float GetCloudsDensity(const atmosphere_params_t &params, Ref::fvec4 local_position, float &out_local_height,
                       float &out_height_fraction, Ref::fvec4 &out_up_vector) {
...
float TraceCloudShadow(const atmosphere_params_t &params, const uint32_t rand_hash, Ref::fvec4 ray_start,
                       const Ref::fvec4 &ray_dir) {
    const Ref::fvec4 clouds_intersection = CloudsIntersection(params, ray_start, ray_dir);
    if (clouds_intersection.get<3>() > 0) {
        const int SampleCount = 32;
        const float StepSize = 16.0f;

        Ref::fvec4 pos = ray_start + Ref::construct_float(rand_hash) * ray_dir * StepSize;
...
float stars_noise(const Ref::fvec4 &p) {
    Ref::fvec4 i = floor(p);
    Ref::fvec4 f = fract(p);
...
Ref::fvec4 SampleMoonTex(Ref::fvec2 uv) {
    uv = uv * Ref::fvec2(MOON_TEX_W, MOON_TEX_H);
    auto iuv0 = Ref::ivec2{uv};
...
Ref::fvec2 SampleCirrusTex(Ref::fvec2 uv) {
    uv = uv * Ref::fvec2(CIRRUS_TEX_W, CIRRUS_TEX_H);
    auto iuv0 = Ref::ivec2{uv};
    iuv0 = clamp(iuv0, Ref::ivec2{0, 0}, Ref::ivec2{CIRRUS_TEX_W - 1, CIRRUS_TEX_H - 1});
...
Ray::Ref::fvec4 Ray::IntegrateOpticalDepth(const atmosphere_params_t &params, const Ref::fvec4 &ray_start,
    ...
    Ref::fvec4 optical_depth = 0.0f;

    for (int i = 0; i < SampleCount; i++) {
        Ref::fvec4 local_pos = ray_start + ray_dir * (i + 0.5f) * step_size, up_vector;
...
Ray::IntegrateScatteringMain(const atmosphere_params_t &params, const Ref::fvec4 &ray_start, const Ref::fvec4 &ray_dir,
    ...
    Ref::fvec4 radiance = 0.0f, multiscat_as_1 = 0.0f;
    ...
    for (int i = 0; i < sample_count; ++i) {
        ...
        const Ref::fvec4 local_position = ray_start + ray_dir * (ray_time - 0.1f * rand_offset * step_size);
        Ref::fvec4 up_vector;
        const float local_height = AtmosphereHeight(params, local_position, up_vector);
        const atmosphere_medium_t medium = SampleAtmosphereMedium(params, local_height);
        const Ref::fvec4 optical_depth = medium.extinction * step_size;
        const Ref::fvec4 local_transmittance = exp(-optical_depth);
        ...
        if (light_dir.get<1>() > -0.025f) {
            ...
            const Ref::fvec4 light_transmittance = SampleTransmittanceLUT(transmittance_lut, uv);
            ...
            const Ref::fvec4 phase_times_scattering =
        ...
        // of 1.0/(4*PI)
        const Ref::fvec4 MS = medium.scattering * 1.0f;
        const Ref::fvec4 MS_int = (MS - MS * local_transmittance) / medium.extinction;
        multiscat_as_1 += inout_transmittance * MS_int;

#if 0
        radiance += inout_transmittance * S * step_size;
#else
        const Ref::fvec4 S_int = (S - S * local_transmittance) / medium.extinction;
    ...
    //
    if (planet_intersection.get<0>() > 0) {
        const Ref::fvec4 local_position = ray_start + ray_dir * planet_intersection.get<0>();
        Ref::fvec4 up_vector;
        const float local_height = AtmosphereHeight(params, local_position, up_vector);
        ...
        const Ref::fvec2 uv =
            LutTransmittanceParamsToUv(params, local_height + params.planet_radius, view_zenith_cos_angle);
        const Ref::fvec4 light_transmittance = SampleTransmittanceLUT(transmittance_lut, uv);
        radiance += Ref::fvec4{params.ground_albedo, Ref::vector_aligned} * saturate(dot(up_vector, light_dir)) *
...
Ray::Ref::fvec4 Ray::IntegrateScattering(const atmosphere_params_t &params, Ref::fvec4 ray_start,
    ...
    const Ref::fvec2 moon_intersection = MoonIntersection(params, ray_start, ray_dir);
    Ref::fvec4 moon_dir = Ref::fvec4{params.moon_dir, Ref::vector_aligned};
    const Ref::fvec4 moon_point = moon_dir * params.moon_distance + 0.5f * light_dir * params.moon_radius;
    ...
    const float moon_costh = dot(ray_dir, moon_dir);
    const Ref::fvec4 moon_phase_w = PhaseWrenninge(moon_costh);
    ...
    Ref::fvec4 total_radiance = 0.0f, total_transmittance = 1.0f;
    ...
    if (planet_intersection.get<0>() < 0 && clouds_intersection.get<1>() > 0 && params.clouds_density > 0.0f &&
        ...
        Ref::fvec4 clouds_ray_start = ray_start + ray_dir * clouds_intersection.get<1>();
        ...
        if (clouds_ray_length > 0.0f) {
            ...
            Ref::fvec4 local_position = clouds_ray_start + ray_dir * Ref::construct_float(rand_hash) * step_size;
            rand_hash = Ref::hash(rand_hash);

            Ref::fvec4 clouds = 0.0f;
            ...
            Ref::fvec4 transmittance_before = total_transmittance;
    ...
    if (planet_intersection.get<0>() < 0 && clouds_intersection.get<1>() > 0 && params.cirrus_clouds_amount > 0.0f &&
        ...
        Ref::fvec2 cirrus_coords =
            3e-4f * Ref::fvec2{params.clouds_offset_z, params.clouds_offset_x} +
            0.8f * (Ref::fvec2{ray_dir.get<2>(), ray_dir.get<0>()}) / (fabsf(ray_dir.get<1>()) + 0.02f);
        ...
        Ref::fvec4 local_position = ray_start + ray_dir * params.cirrus_clouds_height;

        Ref::fvec4 light_transmittance, moon_transmittance;
        {
            Ref::fvec4 up_vector;
            const float local_height = AtmosphereHeight(params, local_position, up_vector);
            {
    ...
    if (planet_intersection.get<0>() < 0 && light_brightness > 0.0f) {
        ...
        Ref::fvec4 main_ray_start = ray_start + ray_dir * clouds_intersection.get<3>();
    ...
    if (light_angle > 0.0f && planet_intersection.get<0>() < 0.0f && light_brightness > 0.0f) {
        ...
        Ref::fvec4 sun_disk = total_transmittance * smoothstep(cos_theta - BlendVal, cos_theta + BlendVal, costh);
    ...
    if (planet_intersection.get<0>() < 0 && moon_intersection.get<0>() > 0 && params.moon_radius > 0.0f &&
        ...
        const Ref::fvec2 uvs = Ref::fvec2(u, theta);
        const Ref::fvec4 albedo = SampleMoonTex(uvs);
...
void Ray::UvToLutTransmittanceParams(const atmosphere_params_t &params, Ref::fvec2 uv, float &view_height,
