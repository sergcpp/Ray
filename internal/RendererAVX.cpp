...
class SIMDPolicy : public SIMDPolicyBase {
  protected:
    static force_inline eRendererType type() { return eRendererType::SIMD_AVX; }
};

RendererBase *CreateRenderer(const settings_t &s, ILog *log) { return new Cpu::Renderer<Avx::SIMDPolicy>(s, log); }
} // namespace Avx
template class Cpu::Renderer<Avx::SIMDPolicy>;
} // namespace Ray
