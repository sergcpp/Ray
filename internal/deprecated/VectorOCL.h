...
template <typename T> class Vector {
  ...
  public:
    Vector(const cl_mem_flags flags, const size_t capacity = 16)
        : Vector(cl::Context::getDefault(), cl::CommandQueue::getDefault(), flags, capacity) {}
    Vector(const cl::Context &context, const cl::CommandQueue &queue, const cl_mem_flags flags,
           const size_t capacity = 16, const size_t max_img_buf_size = 0)
        : context_(context), queue_(queue), max_img_buf_size_(max_img_buf_size), flags_(flags), size_(0),
          cap_(capacity) {
        cl_int error = CL_SUCCESS;
        buf_ = cl::Buffer(context_, flags_, sizeof(T) * cap_, nullptr, &error);
        if (error != CL_SUCCESS) {
            throw std::runtime_error("Cannot allocate OpenCL buffer!");
        }
    }

    const cl::Buffer &buf() const { return buf_; }
    ...
    void Reserve(const size_t req_cap) {
        if (cap_ < req_cap) {
            ...
            if (size_) {
                ...
                if (error != CL_SUCCESS) {
                    throw std::runtime_error("Cannot copy OpenCL buffer!");
                }
            }
            ...
            if (sizeof(T) % 16 == 0 && ((sizeof(T) / 16)) * cap_ <= max_img_buf_size_) {
                img_buf_ =
                    cl::Image1DBuffer(context_, 0, {CL_RGBA, CL_UNSIGNED_INT32}, (sizeof(T) / 16) * cap_, buf_, &error);
                if (error != CL_SUCCESS) {
                    throw std::runtime_error("Cannot create image buffer!");
                }
            }
        }
    }
    ...
    void Append(const T *vec, size_t num) {
        ...
        if (error != CL_SUCCESS) {
            throw std::runtime_error("Cannot write OpenCL buffer!");
        }
...
#endif
        const cl_int error = queue_.enqueueReadBuffer(buf_, CL_TRUE, sizeof(T) * i, sizeof(T), &v);
        if (error != CL_SUCCESS) {
            throw std::runtime_error("Cannot read OpenCL buffer!");
        }
    }
...
#endif
        const cl_int error = queue_.enqueueWriteBuffer(buf_, CL_TRUE, sizeof(T) * i, sizeof(T), &v);
        if (error != CL_SUCCESS) {
            throw std::runtime_error("Cannot write OpenCL buffer!");
        }
    }
...
#endif
        const cl_int error = queue_.enqueueWriteBuffer(buf_, CL_TRUE, sizeof(T) * offset, sizeof(T) * count, p);
        if (error != CL_SUCCESS) {
            throw std::runtime_error("Cannot write OpenCL buffer!");
        }
    }
