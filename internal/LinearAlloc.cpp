...
uint32_t Ray::LinearAlloc::Alloc(const uint32_t req_size, const char *tag) {
    ...
    while (loc_beg < loc_lim && !bitmap_[loc_beg]) {
        ++loc_beg;
    }
...
void Ray::LinearAlloc::Free(const uint32_t offset, const uint32_t size) {
