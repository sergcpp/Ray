...
Ray::eImgUsage Ray::Vk::ImgUsageFromState(const eResState state) { return g_img_usage_per_state[int(state)]; }

Ray::Vk::Image::Image(std::string_view name, Context *ctx, const ImgParams &p, MemAllocators *mem_allocs, ILog *log)
    : ctx_(ctx), name_(name) {
    Init(p, mem_allocs, log);
}

Ray::Vk::Image::Image(std::string_view name, Context *ctx, const void *data, const uint32_t size,
                          const ImgParams &p, Buffer &stage_buf, VkCommandBuffer cmd_buf, MemAllocators *mem_allocs,
                          eImgLoadStatus *load_status, ILog *log)
    : ctx_(ctx), name_(name) {
    Init(data, size, p, stage_buf, cmd_buf, mem_allocs, load_status, log);
}

Ray::Vk::Image::~Image() { Free(); }

Ray::Vk::Image &Ray::Vk::Image::operator=(Image &&rhs) noexcept {
    if (this == &rhs) {
        return (*this);
    }
...
void Ray::Vk::Image::Init(const ImgParams &p, MemAllocators *mem_allocs, ILog *log) {
    InitFromRAWData(nullptr, 0, nullptr, mem_allocs, p, log);
}

void Ray::Vk::Image::Init(const void *data, const uint32_t size, const ImgParams &p, Buffer &sbuf,
                            VkCommandBuffer cmd_buf, MemAllocators *mem_allocs, eImgLoadStatus *load_status,
                            ILog *log) {
...
bool Ray::Vk::Image::Realloc(const int w, const int h, int mip_count, const int samples, const eFormat format,
    ...
    { // create new image
        ...
        img_info.usage = to_vk_image_usage(Bitmask<eImgUsage>{params.usage}, format);
...
void Ray::Vk::Image::InitFromRAWData(Buffer *sbuf, int data_off, VkCommandBuffer cmd_buf, MemAllocators *mem_allocs,
    ...
    { // create default image view(s)
        ...
        if (IsDepthStencilFormat(p.format)) {
            ...
            const VkResult _res = ctx_->api().vkCreateImageView(ctx_->device(), &view_info, nullptr, &depth_only_view);
            if (_res != VK_SUCCESS) {
                log->Error("Failed to create image view!");
                return;
            }
    ...
    { // create new sampler
        ...
        sampler_info.compareEnable = p.sampling.compare != eCompareOp::None ? VK_TRUE : VK_FALSE;
...
void Ray::Vk::Image::SetSubImage(const int level, const int offsetx, const int offsety, const int offsetz,
...
void Ray::Vk::Image::SetSampling(const SamplingParams s) {
    if (handle_.sampler) {
...
void Ray::Vk::CopyImageToImage(VkCommandBuffer cmd_buf, Image &src_tex, const uint32_t src_level,
...
void Ray::Vk::CopyImageToBuffer(const Image &src_tex, const int level, const int x, const int y, const int w,
...
void Ray::Vk::ClearColorImage(Image &tex, const float rgba[4], VkCommandBuffer cmd_buf) {
...
void Ray::Vk::ClearColorImage(Image &tex, const uint32_t rgba[4], VkCommandBuffer cmd_buf) {
    assert(tex.resource_state == eResState::CopyDst);
...
VkFormat Ray::Vk::VKFormatFromFormat(const eFormat format) { return g_formats_vk[size_t(format)]; }
