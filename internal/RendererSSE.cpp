...
void ray::sse::Renderer::RenderScene(const std::shared_ptr<SceneBase> &_s, RegionContext &region) {
    ...
    for (size_t i = 0; i < primary_rays.size(); i++) {
        ...
        __m128 inv_d[3] = { _mm_div_ps(ONE, r.d[0]), _mm_div_ps(ONE, r.d[1]), _mm_div_ps(ONE, r.d[2]) };
