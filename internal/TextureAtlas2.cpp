...
ray::ref::TextureAtlas2::TextureAtlas2(int resx, int resy, int pages_count) : res_{ resx, resy }, res_f_{ (float)resx, (float)resy }, pages_count_(0) {
    if (!Resize(pages_count)) {
        throw std::runtime_error("TextureAtlas cannot be resized!");
    }
}

int ray::ref::TextureAtlas2::Allocate(const pixel_color8_t *data, const int _res[2], int pos[2]) {
...
bool ray::ref::TextureAtlas2::Free(int page, const int pos[2]) {
...
bool ray::ref::TextureAtlas2::Resize(int pages_count) {
